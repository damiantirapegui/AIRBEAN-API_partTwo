...
const loginAdmin = async (req, res) => {
  ...
  try {
    ...
    if (!admin) {
      return res
        .status(401)
        .json({ message: "Accsess denied. Wrong admin account or password." });
    }
...
const logoutAdmin = (req, res) => {
  if (global.currentUser && global.currentUser.role === "admin") {
    ...
    res.status(401).json({
      success: false,
      message: "Unauthorized request, no admin logged in.",
      status: 401,
    });
  }
};

export { createUser, login, logout, loginAdmin, logoutAdmin };
